...
import { typeModel } from "@/utils/const";
...
export const CreateModelDialog: React.FC<PropsCreate> = ({
  children,
  updateView,
}) => {
  ...
  const form = useForm<ModelInterfaces>({
  ...
  function onSubmit(values: ModelInterfaces) {
    setLoadingSave(true);
    ...
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Mostrando datos relacionados con el modelo.
          </DialogDescription>
        </DialogHeader>
        {loadingInit ? null : (
          <Form {...form}>
                    ...
                    <FormItem className="col-span-3 ">
                      ...
                      <FormControl>
                        ...
                        >
                          ...
                          <SelectContent>
                            {sectors?.map((product: SectorInterfaces) => (
                              <SelectItem
                                key={product.id}
...
export const EditModelDialog: React.FC<PropsEdit> = ({
  children,
  id,
  updateView,
  onOpenChange,
  ...
  const fetchModel = async () => {
    setLoadingInit(true);
    try {
      const modelData: ModelInterfaces = await getModelById(id);
